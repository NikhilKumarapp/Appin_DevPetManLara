        ...
        <div class="col-md-12">
            ...
            <div class="card">
                <div class="card-header">
                    {{ trans('global.edit') }} {{ trans('cruds.follow.title_singular') }}
                </div>

                <div class="card-body">
                    <form method="POST" action="{{ route("frontend.follows.update", [$follow->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group">
                            ...
                            <select class="form-control select2" name="follower_id" id="follower_id">
                                @foreach($followers as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('follower_id') ? old('follower_id') : $follow->follower->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control select2" name="following_id" id="following_id" required>
                                @foreach($followings as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('following_id') ? old('following_id') : $follow->following->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
