...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.breed.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.breeds.update", [$breed->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('type') ? 'is-invalid' : '' }}" name="type" id="type" required>
                    ...
                    @foreach(App\Models\Breed::TYPE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('type', $breed->type) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
                @if($errors->has('type'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('breed') ? 'is-invalid' : '' }}" type="text" name="breed" id="breed" value="{{ old('breed', $breed->breed) }}" required>
                @if($errors->has('breed'))
