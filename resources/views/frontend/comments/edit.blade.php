                ...
                <div class="card-body">
                    <form method="POST" action="{{ route("frontend.comments.update", [$comment->id]) }}" enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                            ...
                            <textarea class="form-control" name="description" id="description">{{ old('description', $comment->description) }}</textarea>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control select2" name="answer_id" id="answer_id">
                                @foreach($answers as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('answer_id') ? old('answer_id') : $comment->answer->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
