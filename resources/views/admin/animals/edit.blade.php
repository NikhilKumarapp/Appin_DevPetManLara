    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.animals.update", [$animal->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('title') ? 'is-invalid' : '' }}" type="text" name="title" id="title" value="{{ old('title', $animal->title) }}" required>
                @if($errors->has('title'))
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('gender') ? 'is-invalid' : '' }}" name="gender" id="gender">
                    ...
                    @foreach(App\Models\Animal::GENDER_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('gender', $animal->gender) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('breed') ? 'is-invalid' : '' }}" name="breed_id" id="breed_id" required>
                    @foreach($breeds as $id => $entry)
                        <option value="{{ $id }}" {{ (old('breed_id') ? old('breed_id') : $animal->breed->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('breed'))
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('pet_type') ? 'is-invalid' : '' }}" name="pet_type" id="pet_type" required>
                    ...
                    @foreach(App\Models\Animal::PET_TYPE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('pet_type', $animal->pet_type) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('user') ? 'is-invalid' : '' }}" name="user_id" id="user_id" required>
                    @foreach($users as $id => $entry)
                        <option value="{{ $id }}" {{ (old('user_id') ? old('user_id') : $animal->user->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('user'))
