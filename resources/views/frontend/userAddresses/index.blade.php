        ...
        <div class="col-md-12">
            @can('user_address_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.user-addresses.create') }}">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <div class="table-responsive">
                        <table class=" table table-bordered table-striped table-hover datatable datatable-UserAddress">
                            <thead>
                                <tr>
                                    <th>
                            ...
                            <tbody>
                                @foreach($userAddresses as $key => $userAddress)
                                    <tr data-entry-id="{{ $userAddress->id }}">
                                        ...
                                        <td>
                                            <span style="display:none">{{ $userAddress->default ?? '' }}</span>
                                        ...
                                        <td>
                                            @can('user_address_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.user-addresses.show', $userAddress->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('user_address_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.user-addresses.edit', $userAddress->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('user_address_delete')
                                                <form action="{{ route('frontend.user-addresses.destroy', $userAddress->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('user_address_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.user-addresses.massDestroy') }}",
...
@endcan
  ...
  let table = $('.datatable-UserAddress:not(.ajaxTable)').DataTable({ buttons: dtButtons })
